...
public class Prints {
  ...
  // *************************************************************************** Temps
    ...
    public static Node getNode(String temp) {
      Node n = temps[Integer.parseInt(temp.substring(temp.indexOf("t") + 1))];
      return n;
    }
...
public static void asig(Tuple id, Tuple exp) {
    ...
    public static void checkRangeMulti(String id, String [] dims, int nDim, int i) {
      Node n = VarTable.findNode(id);
      int [] tams = n.getMultiDim();

      DoubleTag aux = new DoubleTag();
      comment("Comprobacion de rango");
      out.println("\tif (" + dims[i] + " < 0) goto " + aux.getT() + ";");
      out.println("\tif (" + tams[i] + " < " + dims[i] + ") goto " + aux.getT() + ";");
      out.println("\tif (" + tams[i] + " == " + dims[i] + ") goto " + aux.getT() + ";");
      _goto(aux.getF());
      label(aux.getT());
      error();
    ...
    public static String getArrayValueMulti(Tuple id, String[] dims, int nDim, boolean isAsig) {
      // Variable Multi
      int [] tams = (VarTable.findNode(id.getA()+"")).getMultiDim();
      String aux = "";
      Tuple t = null;
      for (int i = 0; i < nDim - 1; ++i) {
        if (i == 0) {
          checkRangeMulti(id.getA()+"", dims, nDim, i);
          t = operate(new Tuple(Integer.toString(tams[i+1]), "int"), POR, new Tuple(dims[i], "int"));
          operateEQ(t, MASEQ, new Tuple(dims[i + 1], "int"));
        } else {
          checkRangeMulti(id.getA()+"", dims, nDim, i);
          t = operate(new Tuple(Integer.toString(tams[i+1]), "int"), POR, t);
          operateEQ(t, MASEQ, new Tuple(dims[i + 1], "int"));
        }
      }
      checkRangeMulti(id.getA()+"", dims, nDim, nDim - 1);
      aux = GenTag.genTemp();
      if (!isAsig) {
        out.println("\t" + aux + " = " + id.getA() + "[" + t.getA() + "];");
      }
      return aux;
    }

    public static Tuple<String, String> arrayAsigMulti(Tuple id, Tuple e) {
      String temp = GenTag.genTemp();
      out.println("\t" + id.getA() + "[" + temp + "] = " + e.getA() + ";");
      return new Tuple(id.getA(), id.getB());
    }
