...
public class VarTable {
    ...
    public static int addMultiArray(String arrayName, String [] arrayTams, int tam) {

        int aux = 1;
        for(int i = 0; i < tam; ++i) {
            aux *= Integer.parseInt(arrayTams[i]);
        }
        Node node = new Node(arrayName, scope, type, "array", aux);
        variables.add(node);

        int [] multiDim = new int [tam];
        for (int i = 0; i < tam; ++i) {
            multiDim[i] = Integer.parseInt(arrayTams[i]);
        }
        node.setMultiDim(multiDim);
